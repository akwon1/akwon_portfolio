...
[Does predictive policing enforce racial
bias?](https://youtu.be/ZU_o87c08l4)

# Opening Thought {.center}

**Before we discuss HOW to build predictive policing systems, we need to
ask: SHOULD we?**
...
We will learn the technical methods AND critically interrogate their
use. Both skills are essential for ethical data science.
:::
...
2.  That past patterns predict future crime (they might just predict
    policing)
...
> "Data derived from or influenced by **corrupt, biased, and unlawful
> practices**, including data that has been intentionally manipulated or
> 'juked,' as well as data that is distorted by individual and societal
> biases."
...
-   Unreported crimes (especially in over-policed areas with low police
    trust)
-   Ignored complaints
-   Incomplete records
...
*The Wire* (2004): "If the crime rate doesn't fall, you most certainly
will"
...
-   100+ retired NYPD captains surveyed: **Intense pressure led to stat
    manipulation**
...
> "We would not use data that relates people to predict places—no
> arrests, no social media, no gang status"
...
3.  **Organizationally filtered** - Police priorities, department
    culture
...
**Question:** What would predictive policing look like if we predicted
white-collar crime with the same intensity?

See: [White Collar Crime Risk
Zones](https://whitecollar.thenewinquiry.com/) - A satirical map
...
-   Less than **half** of violent crimes reported to police (DOJ
    estimates)
...
-   No prohibition on using data from 2011-2017 (period of documented
    violations)
-   No requirements for predictive policing oversight
-   Limited transparency about algorithms

**In 2019:** CPD contracted with UChicago Crime Lab to "update"
...
-   A system that accurately predicts where over-policing will occur is
    "accurate"
-   A system that reinforces existing inequalities can be "validated"
-   A system that efficiently targets marginalized communities is
    "effective"
...
**Hypothesis:** Signs of disorder (abandoned cars) predict property
crime (burglaries)
...
Each captures a different aspect of **spatial proximity** to our
indicator variable.
:::
...
Does this actually predict crime? Or does it predict where certain
communities call 311?
...
**Example:** "Chicago burglaries show positive spatial autocorrelation
(I = 0.65)"
:::
...
**Question answered:** "Where are the clusters? Which specific
locations?"
...
**Before calculating spatial autocorrelation, we need to define
"neighbors"**
...
**Cell E's Queen neighbors:** B, D, F, H (edges) + A, C, G, I (corners)
= 8 neighbors
...
**Our fishnet grid uses Queen contiguity** (most common for regular
grids)
...
-   **Quadrant I (High-High):** High crime surrounded by high crime →
    **Hotspot**
-   **Quadrant II (Low-High):** Low crime in high crime area → Outlier
-   **Quadrant III (Low-Low):** Low crime surrounded by low crime →
    **Coldspot**
...
5.  **Gentrifying areas** with new residents calling on existing
    conditions
...
We're using this as a proxy for "disorder" but recognize it's a flawed
proxy
...
1.  **Unobserved heterogeneity:** Some areas have unmeasured crime
    attractors
...
$$\text{Var}(Y_i) = \mu_i + \alpha \mu_i^2$$ **Where:**
...
-   Regional (distance to hotspot cluster) \## Visualizing Distance
    Features
...
-   [x] More spatial detail
-   [x] Better capture local patterns
...
**Example distribution:** - 40% of cells: 0 burglaries - 30% of cells: 1
burglary - 20% of cells: 2-3 burglaries - 10% of cells: 4+ burglaries

**Implications:**

1.  **Poisson handles zeros naturally** (built into distribution)
2.  **Zero-inflation:** If \>60% zeros, consider Zero-Inflated Poisson
    (ZIP)
3.  **For today:** Standard Negative Binomial handles our zeros fine

**Critical interpretation:** Are zeros "true zeros" (no crime) or
"missing data" (unreported)?
...
**Critical question:** Are errors evenly distributed across
neighborhoods?
...
1.  **All crime data is socially constructed** - It reflects policing,
    not just crime
2.  **Historical harms persist in data** - Dirty data trains biased
    systems
3.  **Vendors are not accountable** - Black boxes prevent oversight
4.  **Feedback loops amplify inequality** - Predictions become
    self-fulfilling
5.  **Technical accuracy ≠ social justice** - A "good" model can be
    ethically terrible
