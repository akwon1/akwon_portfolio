...
Notice a big spike in February of 2025 (the 14th - not just Valentine's Day)? Was anybody in Philly last Feb? What closed everything and cancelled my PPA class? ... Hint: Fly Eagles Fly
...
```{r load_indego3}
# Read Q1 2025 data
indego3 <- read_csv("data/indego-trips-2025-q3.csv")
...
# Daily trip counts
daily_trips3 <- indego3 %>%
  group_by(date) %>%
  summarize(trips = n())

ggplot(daily_trips3, aes(x = date, y = trips)) +
  geom_line(color = "#3182bd", linewidth = 1) +
  geom_smooth(se = FALSE, color = "red", linetype = "dashed") +
  labs(
...
```{r hourly_patterns3}
# Average trips by hour and day type
hourly_patterns3 <- indego3 %>%
  group_by(hour, weekend) %>%
  summarize(avg_trips = n() / n_distinct(date)) %>%
  mutate(day_type = ifelse(weekend == 1, "Weekend", "Weekday"))

ggplot(hourly_patterns3, aes(x = hour, y = avg_trips, color = day_type)) +
  geom_line(linewidth = 1.2) +
  scale_color_manual(values = c("Weekday" = "#08519c", "Weekend" = "#6baed6")) +
  labs(
    title = "Average Hourly Ridership Patterns",
...
# Get Philadelphia census tracts/data
glimpse(philly_census)
```
...
```{r join_census_to_stations3}
# Create sf object for stations
stations3_sf <- indego3 %>%
  distinct(start_station, start_lat, start_lon) %>%
  filter(!is.na(start_lat), !is.na(start_lon)) %>%
  st_as_sf(coords = c("start_lon", "start_lat"), crs = 4326)

# Spatial join to get census tract for each station
stations3_census <- st_join(stations3_sf, philly_census, left = TRUE) %>%
  st_drop_geometry()

# Look at the result - investigate whether all of the stations joined to census data -- according to the map above there are stations in non-residential tracts.

stations3_for_map <- indego3 %>%
  distinct(start_station, start_lat, start_lon) %>%
  filter(!is.na(start_lat), !is.na(start_lon)) %>%
  left_join(
    stations_census %>% select(start_station, Med_Inc),
...
stations3_for_map <- indego3 %>%
  ...
  filter(!is.na(start_lat), !is.na(start_lon)) %>%
  left_join(
    stations3_census %>% select(start_station, Med_Inc),
    by = "start_station"
  ) %>%
  mutate(has_census = !is.na(Med_Inc))
...
ggplot() +
  ...
  # Stations with census data (small grey dots)
  geom_point(
    data = stations3_for_map %>% filter(has_census),
    aes(x = start_lon, y = start_lat),
    color = "grey30", size = 1, alpha = 0.6
  ) +
  # Stations WITHOUT census data (red X marks the spot)
...
```{r}
# Identify which stations to keep
valid_stations3 <- stations3_census %>%
  filter(!is.na(Med_Inc)) %>%
  pull(start_station)
...
# Process weather data
weather_processed3 <- weather_data3 %>%
  mutate(
    interval60 = floor_date(valid, unit = "hour"),
    Temperature = tmpf,  # Temperature in Fahrenheit
    Precipitation = ifelse(is.na(p01i), 0, p01i),  # Hourly precip in inches
...
# Check for missing hours and interpolate if needed
weather_complete3 <- weather_processed3 %>%
  complete(interval60 = seq(min(interval60), max(interval60), by = "hour")) %>%
  fill(Temperature, Precipitation, Wind_Speed, .direction = "down")

# Look at the weather
summary(weather_complete3 %>% select(Temperature, Precipitation, Wind_Speed))
```

## Visualize Weather Patterns

Who is glad it's Fall?

```{r visualize-weather3}
#| message: false
#| warning: false
#| 
ggplot(weather_complete3, aes(x = interval60, y = Temperature)) +
  geom_line(color = "#3182bd", alpha = 0.7) +
  geom_smooth(se = FALSE, color = "red") +
  labs(
    title = "Philadelphia Temperature - Q3 2025",
    subtitle = "Late summer to early fall transition",
    x = "Date",
    y = "Temperature (°F)"
  ) +
  plotTheme
...
# How many unique stations?
length(unique(trips_panel3$start_station))

# How many unique hours?
length(unique(trips_panel3$interval60))

hist(trips_panel3$Trip_Count)
```
...
```{r complete-panel3}
# Calculate expected panel size
n_stations <- length(unique(trips_panel3$start_station))
n_hours <- length(unique(trips_panel3$interval60))
expected_rows <- n_stations * n_hours
...
# Create complete panel
study_panel3 <- expand_grid(
  interval60 = unique(trips_panel3$interval60),
  start_station = unique(trips_panel3$start_station)
) %>%
  # Join trip counts
  left_join(trips_panel3 %>% 
              select(interval60, start_station, Trip_Count),
            by = c("interval60", "start_station")) %>%
  # Replace NA trip counts with 0
  mutate(Trip_Count = replace_na(Trip_Count, 0))

glimpse(study_panel3)
hist(study_panel3$Trip_Count)

# Fill in station attributes (they're the same for all hours)
station_attributes3 <- trips_panel3 %>%
  group_by(start_station) %>%
  summarize(
    start_lat = first(start_lat),
    start_lon = first(start_lon),
...
glimpse(station_attributes3)

study_panel3 <- study_panel3 %>%
  left_join(station_attributes, by = "start_station")

glimpse(study_panel3)

# Verify we have complete panel
cat("Complete panel rows:", format(nrow(study_panel3), big.mark = ","), "\n")
```

## Add Time Features

```{r add_time_features3}
study_panel3 <- study_panel3 %>%
  mutate(
    week = week(interval60),
    month = month(interval60, label = TRUE),
...
```{r join_weather3}
study_panel3 <- study_panel3 %>%
  left_join(weather_complete3, by = "interval60")

glimpse(study_panel3)

# Check for missing values
summary(study_panel3 %>% select(Trip_Count, Temperature, Precipitation))
```
...
# Remove rows with NA lags (first 24 hours for each station)
study_panel_complete3 <- study_panel3 %>%
  filter(!is.na(lag1day))

cat("Rows after removing NA lags:", format(nrow(study_panel_complete3), big.mark = ","), "\n")
```
...
```{r lag_correlations3}
# Sample one station to visualize
example_station3 <- study_panel_complete3 %>%
  filter(start_station == first(start_station)) %>%
  head(168)  # One week

# Plot actual vs lagged demand
ggplot(example_station3, aes(x = interval60)) +
  geom_line(aes(y = Trip_Count, color = "Current"), linewidth = 1) +
  geom_line(aes(y = lag1Hour, color = "1 Hour Ago"), linewidth = 1, alpha = 0.7) +
  geom_line(aes(y = lag1day, color = "24 Hours Ago"), linewidth = 1, alpha = 0.7) +
  scale_color_manual(values = c(
...
# Train on weeks 26-35 (Jul 1 - mid August)
# Test on weeks 36-39 (mid August - September)

# Which stations have trips in BOTH early and late periods?
early_stations3 <- study_panel_complete3 %>%
  filter(week < 36) %>%
  filter(Trip_Count > 0) %>%
  distinct(start_station) %>%
  pull(start_station)

late_stations3 <- study_panel_complete3 %>%
  filter(week >= 36) %>%
  filter(Trip_Count > 0) %>%
  distinct(start_station) %>%
  pull(start_station)
...
# Keep only stations that appear in BOTH periods
common_stations3 <- intersect(early_stations3, late_stations3)


# Filter panel to only common stations
study_panel_complete3 <- study_panel_complete3 %>%
  filter(start_station %in% common_stations3)

# NOW create train/test split
train3 <- study_panel_complete3 %>%
  filter(week < 36)

test3 <- study_panel_complete3 %>%
  filter(week >= 36)

cat("Training observations:", format(nrow(train3), big.mark = ","), "\n")
cat("Testing observations:", format(nrow(test3), big.mark = ","), "\n")
cat("Training date range:", min(train3$date), "to", max(train3$date), "\n")
cat("Testing date range:", min(test3$date), "to", max(test3$date), "\n")
...
# Set contrasts to treatment coding (dummy variables)
contrasts(train3$dotw_simple) <- contr.treatment(7)
...
# Summary too long with all station dummies, just show key metrics
cat("Model 4 R-squared:", summary(model4_3)$r.squared, "\n")
cat("Model 4 Adj R-squared:", summary(model4_3)$adj.r.squared, "\n")
```
...
cat("Model 5 R-squared:", summary(model5_3)$r.squared, "\n")
cat("Model 5 Adj R-squared:", summary(model5_3)$adj.r.squared, "\n")
```
...
# Set contrasts to treatment coding (dummy variables)
contrasts(test3$dotw_simple) <- contr.treatment(7)

test3 <- test3 %>%
  mutate(
    pred1 = predict(model1_3, newdata = test3),
    pred2 = predict(model2_3, newdata = test3),
    pred3 = predict(model3_3, newdata = test3),
    pred4 = predict(model4_3, newdata = test3),
    pred5 = predict(model5_3, newdata = test3)
  )

# Calculate MAE for each model
mae_results3 <- data.frame(
  Model = c(
    "1. Time + Weather",
    "2. + Temporal Lags",
    "3. + Demographics",
  ...
  MAE = c(
    mean(abs(test3$Trip_Count - test3$pred1), na.rm = TRUE),
    mean(abs(test3$Trip_Count - test3$pred2), na.rm = TRUE),
    mean(abs(test3$Trip_Count - test3$pred3), na.rm = TRUE),
    mean(abs(test3$Trip_Count - test3$pred4), na.rm = TRUE),
    mean(abs(test3$Trip_Count - test3$pred5), na.rm = TRUE)
  )
)
...
```{r compare_models3}
ggplot(mae_results3, aes(x = reorder(Model, -MAE), y = MAE)) +
  geom_col(fill = "#3182bd", alpha = 0.8) +
  geom_text(aes(label = round(MAE, 2)), vjust = -0.5) +
  labs(
    title = "Model Performance Comparison",
...
# Scatter plot by time and day type
ggplot(test3, aes(x = Trip_Count, y = pred2)) +
  geom_point(alpha = 0.2, color = "#3182bd") +
  geom_abline(slope = 1, intercept = 0, color = "red", linewidth = 1) +
  geom_smooth(method = "lm", se = FALSE, color = "darkgreen") +
  facet_grid(weekend ~ time_of_day) +
...
ggplot(temporal_errors3, aes(x = time_of_day, y = MAE, fill = day_type)) +
  geom_col(position = "dodge") +
  scale_fill_manual(values = c("Weekday" = "#08519c", "Weekend" = "#6baed6")) +
  labs(
    title = "Prediction Errors by Time Period",
...
```{r errors_demographics3}
#| message: false
#| warning: false

# Join demographic data to station errors
station_errors_demo3 <- station_errors3 %>%
  left_join(
    station_attributes %>% select(start_station, Med_Inc, Percent_Taking_Transit, Percent_White),
    by = "start_station"
  ) %>%
...
# Create plots
p1_3 <- ggplot(station_errors_demo3, aes(x = Med_Inc, y = MAE)) +
  geom_point(alpha = 0.5, color = "#3182bd") +
  geom_smooth(method = "lm", se = FALSE, color = "red") +
  scale_x_continuous(labels = scales::dollar) +
  labs(title = "Errors vs. Median Income", x = "Median Income", y = "MAE") +
...
p2_3 <- ggplot(station_errors_demo3, aes(x = Percent_Taking_Transit, y = MAE)) +
  geom_point(alpha = 0.5, color = "#3182bd") +
  geom_smooth(method = "lm", se = FALSE, color = "red") +
  labs(title = "Errors vs. Transit Usage", x = "% Taking Transit", y = "MAE") +
  plotTheme

p3_3 <- ggplot(station_errors_demo3, aes(x = Percent_White, y = MAE)) +
  geom_point(alpha = 0.5, color = "#3182bd") +
  geom_smooth(method = "lm", se = FALSE, color = "red") +
  labs(title = "Errors vs. Race", x = "% White", y = "MAE") +
  plotTheme
...
# Convert to sf using lon/lat columns
train3_sf <- train3 %>%
  st_as_sf(coords = c("start_lon", "start_lat"), crs = 4326)

train3_sf <- train3_sf %>%
  mutate(
    dist_downtown = as.numeric(st_distance(geometry, city_hall))
  ) # Calculate distance to city hall

# Landmarks: https://opendataphilly.org/datasets/city-landmarks/
landmarks <- st_read("C:/Users/angel/Documents/akwon_portfolio/labs/lab_5/data/Landmark_Points.shp") %>%
  st_transform(4326)

station_sf <- station_attributes %>%
  st_as_sf(coords = c("start_lon", "start_lat"), crs = 4326)
station_landmarks_dist <- st_distance(station_sf, landmarks) # Calculate distance to landmarks

get_knn_distance <- function(station_landmarks_dist, k) {
  apply(station_landmarks_dist, 1, function(distances){
...
station_sf <- station_sf %>%
  mutate(
    stops_nn1 = get_knn_distance(station_landmarks_dist, k= 1)
  ) # Create features

train3_sf <- st_join(train3_sf, station_sf, left = TRUE)

train3_sf <- train3_sf %>%
  rename(Med_Inc = Med_Inc.x, Percent_Taking_Transit = Percent_Taking_Transit.x, 
         Percent_White = Percent_White.x, start_station = start_station.x)
```
...
# Create station-level lookup table from train3
station_features <- train3_sf %>%
  group_by(start_station) %>%
  summarize(
    stops_nn1 = first(stops_nn1),
    dist_downtown = first(dist_downtown),
    .groups = "drop"
  )

# Join onto test3 (trip-level data)
test3 <- test3 %>%
  left_join(station_features, by = "start_station")

test3 <- test3 %>%
  mutate(
    pred6 = predict(model6, newdata = test3)
  )
...
mae_results3 <- data.frame(
  Model = c(
    ...
    "4. + Station FE",
    "5. + Rush Hour Interaction",
    "6. + Distance to downtown + Distance to nearest landmark"
  ),
  MAE = c(
    mean(abs(test3$Trip_Count - test3$pred1), na.rm = TRUE),
    mean(abs(test3$Trip_Count - test3$pred2), na.rm = TRUE),
    ...
    mean(abs(test3$Trip_Count - test3$pred4), na.rm = TRUE),
    mean(abs(test3$Trip_Count - test3$pred5), na.rm = TRUE),
    mean(abs(test3$Trip_Count - test3$pred6), na.rm = TRUE)
...
I would say that the final MAE of 0.77 means that the model is still not quite good enough for Indego to use, as that is still quite high. This large MAE means that Indego may overstock or understcok certain bike stations, and that may lead to affecting the customers who need the bikes for thei commute or transportation. I would only recommend deploying this system after more analysis is done and the MAE is below 0.5.

It seems that the model is less effective in predicting rush hour predictions, which is interesting as that seems like something that is a given. It is possible that people's commute patterns have changed drastically since remote and hybrid work has become commonplace and this muddies the data a bit. This would disproportionately afffect neighborhoods of lower income which may offer less remote jobs and therefore, the other areas with less travel, or more sporadic travel, may muddy the data on their uses of Indego bikes. I would ensure that there are more spatial demographic data in the model to ensure that everyone is represented.

This model overall mostly predicts that people travel in a set pattern, but personally I can say that me and many of my peers hold very irregular patterns. This model seems to have difficulty predicting sporadic travel, which accounts for everyone outside of a 9-5 job, including full-time students, part-time workers, unemployed and the retired. More training would be necessary for the model to become stronger.

When collecting data, I think another aspect that could be analyzed is how many times someone clicks to check on a station on the app to check for bike availabilities, and if they find the one they are looking for. This may account for undercounted demand that is not represented by a bike ride, as there was no bike ride.
